...
model = BertForYieldPretrainModel(model_type='bert', 
                                  model_name='./outputs/egret_pretrain_stage_1/bestoutput_t', 
                                  args=args, 
                                  use_cuda=True if not debug else False, 
                                  cuda_device=0, 
                                  train_from_mlm=True)
train_file = './dataset/pretrain_data/aug_pis_train_dataset_sub_t.csv'
eval_file = './dataset/pretrain_data/aug_pis_eval_dataset_sub_t.csv'
